...
**Note**: The `SKIP_AUTH=1` environment variable is available for a simpler (unrecommended) development setup without oauth-proxy, but `make local-dev` uses the full authentication flow with mock-oauth-proxy to accurately mirror production.

The mock-oauth-proxy:
- Accepts Basic Auth credentials
- Validates against [YAML user configuration](hack/local/mock-oauth-proxy-config.yaml)
...
- Generated automatically by `local-dev.sh`
...
The e2e test suite (`make e2e`) uses the same architecture as local development:
